...
def lock_down_vectorai_firewall():
    """Add Windows Firewall rules that restrict vector-ai port 8000 to loopback
    and Tailscale only. Runs at supervisor startup (which is elevated), so no
    UAC prompt is needed. Safe to call repeatedly — removes old rules first.

    Without this, binding on 0.0.0.0 would expose port 8000 on all LAN
    interfaces. With these rules, only 127.0.0.1 (chipper) and the
    Tailscale CGNAT range (100.64.0.0/10, the Pi) can reach it.
    """
    venv_py = str(VECTORAI_DIR / "venv" / "Scripts" / "python.exe")
    if not Path(venv_py).exists():
        log(f"firewall: venv python not found at {venv_py} — skipping")
        return

    ps_script = f"""
$p = '{venv_py}'
# Remove stale vector-ai rules
Get-NetFirewallRule -DisplayName 'vector-ai-8000-*' -ErrorAction SilentlyContinue | Remove-NetFirewallRule

# Allow from loopback (chipper at 127.0.0.1)
New-NetFirewallRule -DisplayName 'vector-ai-8000-loopback' -Direction Inbound -Action Allow `
    -Protocol TCP -LocalPort 8000 -RemoteAddress '127.0.0.1' -Program $p -Profile Any | Out-Null

# Allow from Tailscale CGNAT range (all Tailscale peers including the Pi)
New-NetFirewallRule -DisplayName 'vector-ai-8000-tailscale' -Direction Inbound -Action Allow `
    -Protocol TCP -LocalPort 8000 -RemoteAddress '100.64.0.0/10' -Program $p -Profile Any | Out-Null

# Block everything else on port 8000
New-NetFirewallRule -DisplayName 'vector-ai-8000-block' -Direction Inbound -Action Block `
    -Protocol TCP -LocalPort 8000 -Program $p -Profile Any | Out-Null

Write-Host 'vector-ai firewall rules applied'
"""
    try:
        ...
        if out:
            log(f"firewall: {out}")
        if err:
            log(f"firewall WARNING: {err}")
    except Exception as e:
        log(f"firewall: failed to apply rules: {e}")
...
class Supervisor:
    ...
    def startup(self):
        ...
        self.start_vectorai()
        self.start_chipper()
        # Pre-warm the main model so the first voice request isn't a 60-105s
        # cold start. Runs as a daemon thread — startup returns immediately.
    ...
    def _prewarm_model(self):
        ...
        try:
            ...
            log(f"pre-warm complete — {model} ready")
        except Exception as e:
            log(f"pre-warm warning: {e}")
    ...
    def shutdown_all(self):
        ...
        model_to_stop = os.environ.get("OLLAMA_MODEL", "llama3.3:70b")
        try:
            subprocess.run(["ollama", "stop", model_to_stop],
